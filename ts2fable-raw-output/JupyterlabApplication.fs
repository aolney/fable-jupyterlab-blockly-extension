...
type CommandRegistry = ``@phosphor_commands``.CommandRegistry //AMO bacticked variables starting with @ throughout
type ServerConnection = ``@jupyterlab_services``.ServerConnection
type ServiceManager = ``@jupyterlab_services``.ServiceManager
...
type [<AllowNullLiteral>] IConnectionLost =
    [<Emit "$0($1...)">] abstract Invoke: manager: ServiceManager.IManager * err: ServerConnection.NetworkError -> Promise<unit>
...
type CommandLinker = ``@jupyterlab_apputils``.CommandLinker
type DocumentRegistry = ``@jupyterlab_docregistry``.DocumentRegistry
// type ServiceManager = ``@jupyterlab_services``.ServiceManager //AMO duplicate type
type IIterator = ``@phosphor_algorithm``.IIterator
type Application = ``@phosphor_application``.Application
type IPlugin = ``@phosphor_application``.IPlugin
...
module JupyterFrontEnd =
    ...
    type [<AllowNullLiteral>] IOptions<'T, 'U> =
        inherit Application.IOptions<'T>
        /// The document registry instance used by the application.
    ...
    type [<AllowNullLiteral>] IShell =
        ...
        abstract add: widget: Widget * ?area: string * ?options: DocumentRegistry.IOpenOptions -> unit
...
type DockLayout = ``@phosphor_widgets``.DockLayout
type DockPanel = ``@phosphor_widgets``.DockPanel
type FocusTracker = ``@phosphor_widgets``.FocusTracker
...
module ILabShell =
    ...
    type [<AllowNullLiteral>] IMainArea =
        ...
        abstract dock: DockLayout.ILayoutConfig option
        /// The document mode (i.e., multiple/single) of the main dock panel.
        abstract mode: DockPanel.Mode option
...
type [<AllowNullLiteral>] LabShell =
    ...
    abstract add: widget: Widget * ?area: ILabShell.Area * ?options: DocumentRegistry.IOpenOptions -> unit
...
module JupyterLab =
    ...
    type [<AllowNullLiteral>] IInfo =
        ...
        abstract mimeExtensions: ResizeArray<IRenderMime.IExtensionModule>
...
type IDataConnector = ``@jupyterlab_coreutils``.IDataConnector
type IRestorer = ``@jupyterlab_coreutils``.IRestorer
// type CommandRegistry = ``@phosphor_commands``.CommandRegistry
type ReadonlyJSONValue = ``@phosphor_coreutils``.ReadonlyJSONValue
// type Token = ``@phosphor_coreutils``.Token
// type Widget = ``@phosphor_widgets``.Widget
// type ILabShell = __shell.ILabShell
...
type [<AllowNullLiteral>] ILayoutRestorer =
    ...
    abstract restore: tracker: WidgetTracker<'T> * options: IRestorer.IOptions<'T> -> Promise<obj option>
...
type [<AllowNullLiteral>] LayoutRestorer =
    ...
    /// <param name="tracker">- The widget tracker whose widgets will be restored.</param>
    /// <param name="options">- The restoration options.</param>
    abstract restore: tracker: WidgetTracker * options: IRestorer.IOptions<Widget> -> Promise<obj option>
...
type IWidgetTracker = ``@jupyterlab_apputils``.IWidgetTracker
// type WidgetTracker = ``@jupyterlab_apputils``.WidgetTracker
type MimeDocument = ``@jupyterlab_docregistry``.MimeDocument
// type IRenderMime = ``@jupyterlab_rendermime_interfaces``.IRenderMime
// type Token = ``@phosphor_coreutils``.Token
...
type [<AllowNullLiteral>] IMimeDocumentTracker =
    inherit IWidgetTracker<MimeDocument>
// type CommandRegistry = ``@phosphor_commands``.CommandRegistry
// type Token = ``@phosphor_coreutils``.Token
// type IDisposable = ``@phosphor_disposable``.IDisposable
// type ISignal = ``@phosphor_signaling``.ISignal
