...
type Kernel = ``@jupyterlab_services``.Kernel
type KernelMessage = ``@jupyterlab_services``.KernelMessage
type Session = ``@jupyterlab_services``.Session
type IterableOrArrayLike = ``@phosphor_algorithm``.IterableOrArrayLike
...
type [<AllowNullLiteral>] IClientSession =
    ...
    abstract kernelChanged: ISignal<IClientSession, Session.IKernelChangedArgs>
    /// A signal emitted when the kernel status changes.
    abstract statusChanged: ISignal<IClientSession, Kernel.Status>
    /// A signal emitted for a kernel messages.
    abstract iopubMessage: ISignal<IClientSession, KernelMessage.IMessage>
    /// A signal emitted for an unhandled kernel message.
    abstract unhandledMessage: ISignal<IClientSession, KernelMessage.IMessage>
    /// A signal emitted when a session property changes.
    abstract propertyChanged: ISignal<IClientSession, U3<string, string, string>>
    /// The current kernel associated with the document.
    abstract kernel: Kernel.IKernelConnection option
...
type [<AllowNullLiteral>] ClientSession =
    ...
    /// A signal emitted when the kernel changes.
    abstract kernelChanged: ISignal<ClientSession, Session.IKernelChangedArgs>
    /// A signal emitted when the status changes.
    abstract statusChanged: ISignal<ClientSession, Kernel.Status>
    /// A signal emitted for iopub kernel messages.
    abstract iopubMessage: ISignal<ClientSession, KernelMessage.IIOPubMessage>
    /// A signal emitted for an unhandled kernel message.
    abstract unhandledMessage: ISignal<ClientSession, KernelMessage.IMessage>
    /// A signal emitted when a session property changes.
    ...
    abstract manager: Session.IManager
...
module ClientSession =
    ...
    type [<AllowNullLiteral>] IExports =
        ...
        abstract restartKernel: kernel: Kernel.IKernelConnection -> Promise<bool>
    ...
    type [<AllowNullLiteral>] IOptions =
        ...
        abstract manager: Session.IManager with get, set
    ...
    type [<AllowNullLiteral>] IKernelSearch =
        /// The Kernel specs.
        abstract specs: Kernel.ISpecModels option with get, set
        /// The kernel preference.
        abstract preference: IClientSession.IKernelPreference with get, set
        /// The current running sessions.
        abstract sessions: IterableOrArrayLike<Session.IModel> option with get, set
type MimeData = ``@phosphor_coreutils``.MimeData
...
type Message = ``@phosphor_messaging``.Message
type ISignal = ``@phosphor_signaling``.ISignal
...
module Collapse =
    ...
    type [<AllowNullLiteral>] IOptions<'T> =
        inherit Widget.IOptions
        abstract widget: 'T with get, set
        abstract collapsed: bool option with get, set
type JSONObject = ``@phosphor_coreutils``.JSONObject
type IDisposable = ``@phosphor_disposable``.IDisposable
...
type IDisposable = ``@phosphor_disposable``.IDisposable
type CommandPalette = ``@phosphor_widgets``.CommandPalette
...
type [<AllowNullLiteral>] IPaletteItem =
    inherit CommandPalette.IItemOptions
...
type IRestorable = ``@jupyterlab_coreutils``.IRestorable
type IDisposable = ``@phosphor_disposable``.IDisposable
type ISignal = ``@phosphor_signaling``.ISignal
type Widget = ``@phosphor_widgets``.Widget
...
type [<AllowNullLiteral>] WidgetTracker<'T> =
    ...
    abstract restore: options: IRestorable.IOptions<'T> -> Promise<obj option>
...
type Message = ``@phosphor_messaging``.Message
type ISignal = ``@phosphor_signaling``.ISignal
type Signal = ``@phosphor_signaling``.Signal
type Widget = ``@phosphor_widgets``.Widget
...
type ReactRenderElement =
    U2<Array<React.ReactElement<obj option>>, React.ReactElement<obj option>>
...
type [<AllowNullLiteral>] UseSignal<'SENDER, 'ARGS> =
    inherit React.Component<IUseSignalProps<'SENDER, 'ARGS>, IUseSignalState<'SENDER, 'ARGS>>
...
type IIterator = ``@phosphor_algorithm``.IIterator
type CommandRegistry = ``@phosphor_commands``.CommandRegistry
type Message = ``@phosphor_messaging``.Message
type Widget = ``@phosphor_widgets``.Widget
type IClientSession = __clientsession.IClientSession
type ReadonlyJSONObject = ``@phosphor_coreutils``.ReadonlyJSONObject
...
type IChangedArgs = ``@jupyterlab_coreutils``.IChangedArgs
type Token = ``@phosphor_coreutils``.Token
type IDisposable = ``@phosphor_disposable``.IDisposable
type ISignal = ``@phosphor_signaling``.ISignal
...
type IChangedArgs = ``@jupyterlab_coreutils``.IChangedArgs
type ISettingRegistry = ``@jupyterlab_coreutils``.ISettingRegistry
type IDisposable = ``@phosphor_disposable``.IDisposable
type Widget = ``@phosphor_widgets``.Widget
type ISignal = ``@phosphor_signaling``.ISignal
